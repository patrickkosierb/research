...
% inputs
T_ext_input = @(t) 0;
T_motor_input = @(t) 0; %* (heaviside(t - 1.2) - heaviside(t - 2.5));
...
f2 = @(x,u) [  x(1)+x(2)*T;
              x(2)+(( r*cos(x(3))/( l+I_b/(l*m_b) ) * (m_b*g*l*sin(x(3))) - l*r*m_b*x(4)^2*sin(x(3)) + u) / ( m_b*r^2 + 2*m_w*r^2 + 2*I_w - m_b*r^2*cos(x(3))^2/(1 + I_b/(l^2*m_b)) ))*T;
              x(3)+x(4)*T;
              x(4)+ T*(( l*r*m_b*( r*cos(x(3))/( l+I_b/(l*m_b) ) * (m_b*g*l*sin(x(3))) - l*r*m_b*x(4)^2*sin(x(3)) + u) / ( m_b*r^2 + 2*m_w*r^2 + 2*I_w - m_b*r^2*cos(x(3))^2/(1 + I_b/(l^2*m_b)) )*cos(x(3))+ m_b*g*l*sin(x(3)) ) / (m_b*l^2 + I_b))*T;
             ]
...
for k = 1:n_iter
    ...
    % Prediction step
    x_pred = f2(x_est(:,k),u_input); % Predict the next state
